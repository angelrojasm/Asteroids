...
TextRenderer::TextRenderer(TTF_Font* newFont, SDL_Color newColor)
{
	Font = newFont;
	Color = newColor;
...
void::TextRenderer::TextRenderInit()
{
	if (TTF_Init() == -1) {
		SDL_Log("TTF_Init: %s\n", TTF_GetError());
...
void TextRenderer::RenderText(std::string message, SDL_Color color, float x, float y, int size)
{
	...
	glBegin(GL_QUADS); {
		...
		glTexCoord2d(1.0f, 1.0); glVertex3f(0.0f + surface->w, 0.0f, 0.0f);
